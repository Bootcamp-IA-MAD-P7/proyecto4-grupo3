...
df = df.with_columns(
    pl.col("provincia").replace_strict(provincia_to_ccaa, default=None).alias("cc_aa")
)

df = df.select("provincia", "cc_aa", pl.exclude(["provincia", "cc_aa"]))
...
# %%
df = df.with_columns(
    pl.col("velocidad_viento_media").str.replace(",", ".").cast(pl.Float64)
)

# %%
df = df.with_columns(
    pl.col("velocidad_viento_media").fill_null(
        pl.col("velocidad_viento_media").median()
    )
)
...
# %%
df = df.with_columns(pl.col("precipitacion").str.replace(",", ".").cast(pl.Float64))
...
df = df.with_columns(
    ...
    .dt.month()
    .cast(pl.Int64)
    .replace_strict(meses)
    .alias("mes")
).select(pl.col("fecha"), pl.col("mes"), pl.all().exclude("fecha", "mes"))
...
# %%
heatmap_data = (
    df_pd.groupby(["provincia", "mes"])["temperatura_media"]
    .mean()
    .reset_index()
...
alt.Chart(heatmap_data).mark_rect().encode(
    ...
    title="Average Temperature: Province x Month", width=600, height=600
).interactive().show()
...
alt.Chart(heatmap_precip_mean).mark_rect().encode(
    ...
    title="Average Precipitation: Province x Month", width=600, height=600
).interactive().show()
...
alt.Chart(heatmap_viento_mean).mark_rect().encode(
    ...
    title="Average Wind Speed: Province x Month", width=600, height=600
).interactive().show()
...
scatter_precip = scatter_precip.with_columns(
    [pl.col("provincia").cast(pl.String), pl.col("mes").cast(pl.String)]
)
...
alt.Chart(scatter_precip).mark_circle(size=60, opacity=0.7).encode(
    ...
    color=alt.Color(
        "mes:N", title="Month", sort=list(month_order), scale=alt.Scale(scheme="turbo")
    ),
    tooltip=[
        "mes",
        "provincia",
...
).add_params(selector_precip).transform_filter(selector_precip).properties(
    title="Temperature vs Precipitation by Province & Month", width=600, height=400
).interactive().show()
...
).add_params(selector_viento).transform_filter(selector_viento).properties(
    title="Temperature vs Wind Speed by Province & Month", width=600, height=400
).interactive().show()
...
alt.Chart(corr_melted).mark_rect().encode(
    ...
    color=alt.Color(
        "corr:Q", title="Correlation", scale=alt.Scale(scheme="rainbow", domain=[-1, 1])
    ),
    tooltip=["var1", "var2", alt.Tooltip("corr", format=".2f")],
).properties(
    title="Correlation Heatmap (Numeric Variables)", width=500, height=500
).interactive().show()
...
canarias.plot(
    ...
    ax=ax_inset,
    cmap=plt.cm.rainbow,
    vmin=map_df["viento_medio"].min(),
    vmax=map_df["viento_medio"].max(),
)
...
for var, label in variables.items():
    ...
    canarias = map_df[
        map_df["provincia"].isin(["LAS PALMAS", "SANTA CRUZ DE TENERIFE"])
    ]
    peninsula = map_df[
        ~map_df["provincia"].isin(["LAS PALMAS", "SANTA CRUZ DE TENERIFE"])
    ]
    ...
    cmap = LinearSegmentedColormap.from_list(
        "custom", ["#FF477B", "#8858C8", "#C0F7F4", "#5ECFB8"]
    )

    fig, ax = plt.subplots(1, 1, figsize=(12, 8))
    map_df.plot(
        column="valor", ax=ax, legend=True, cmap=cmap, legend_kwds={"label": label}
    )
    ...
    canarias.plot(
        ...
        ax=ax_inset,
        cmap=cmap,
        vmin=map_df["valor"].min(),
        vmax=map_df["valor"].max(),
    )
    ax_inset.set_axis_off()
